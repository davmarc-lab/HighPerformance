...
#pragma omp parallel default(none) shared(chunk_size, p, vin, vout, n) private(i)
{
    int my_id = omp_get_thread_num();

    int start = my_id * chunk_size;
    int stride = p * chunk_size;
...
void do_dynamic(const int *vin, int *vout, int n)
{
    ...
    */
    ...
    const int chunk_size = 1;
    int idx = 0;
#pragma omp parallel default(none) shared(chunk_size, idx, vin, vout, n) private(i)
{
    int my_idx;
    do {
#pragma omp critical
