...
void my_Bcast(int *v)
{
    ...
    if (my_rank > 0)
    {
        ...
        const int dest1 = my_rank * 2 + 1 < comm_sz ? 2 * my_rank + 1 : MPI_PROC_NULL;
        const int dest2 = my_rank * 2 + 2 < comm_sz ? 2 * my_rank + 2 : MPI_PROC_NULL;

        MPI_Send(v, 1, MPI_INT, dest1, 0, MPI_COMM_WORLD);
        MPI_Send(v, 1, MPI_INT, dest2, 0, MPI_COMM_WORLD);
    } else {
        MPI_Send(v, 1, MPI_INT, 2 * my_rank + 1, 0, MPI_COMM_WORLD);
        MPI_Send(v, 1, MPI_INT, 2 * my_rank + 2, 0, MPI_COMM_WORLD);
    }
}
...
int main( int argc, char *argv[] )
{
    ...
    printf("BEFORE: value 'v' at rank %d = %d\n", my_rank, v);
