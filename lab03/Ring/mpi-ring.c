...
int main( int argc, char *argv[] )
{
    ...
    if (my_rank == 0) 
    {
        ...
        const int pred = my_rank - 1 == 0 ? comm_sz : my_rank - 1;
        const int succ = my_rank + 1 == comm_sz ? 0 : my_rank + 1;
        printf("DEBUG: pred = %d, succ = %d\n", pred, succ);

        MPI_Recv(&value, 1, MPI_INT, pred, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        value++;
        printf("DEBUG: value = %d\n", value);
        MPI_Send(&value, 1, MPI_INT, succ, 0, MPI_COMM_WORLD);
