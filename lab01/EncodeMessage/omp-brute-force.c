...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel default(none) shared(n, msglen, KEY_LEN, enc, check, CHECK_LEN, found) private(k)
    {
        char key[KEY_LEN + 1];   /* sprintf will output the trailing \0, so we need one
                                    byte more for the key */
